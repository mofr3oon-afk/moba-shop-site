...
export default async function handler(req,res){
  try{
    ...
    // V185: الكوبون يعتمد فقط على الأكواد الموجودة حاليا في لوحة الأدمن (settings.coupon_rules).
    // أي كوبون قديم في جدول coupons لن يشتغل لو اتحذف من اللوحة.
    const c = await readCouponRuleFromSettings(code);
    if(!c) return json(res,404,{ok:false,error:'الكود ده غير موجود أو تم حذفه من لوحة الأدمن'});
