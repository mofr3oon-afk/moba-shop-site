...
function addProduct(game,section){
  ...
  const price=Number(prompt('السعر الأساسي؟')||0); if(!Number.isFinite(price)) return;
  const salePrice=Number(prompt('سعر العرض لو موجود؟ اتركه 0 لو مفيش عرض')||0);
  local.products=[...(local.products||[]),{
    id:'p_'+Date.now(),
    game,
    section,
    ...
    salePrice:Math.max(0,Number.isFinite(salePrice)?salePrice:0),
    amount:0,
    unit:'',
    order:99,
    status:'available',
