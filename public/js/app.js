...
function renderSkeleton(label='جاري تحميل بيانات MOBA SHOP...'){
  app.innerHTML = `<section class="section skeleton-shell" aria-live="polite" aria-busy="true">
    ...
    <p class="skeleton-label">${esc(label)}</p>
  </section>`;
  setActiveNav();
  updateCartBubble();
}
...
function showFieldError(form, selector, message){
  ...
  const holder = input?.closest('.field') || input?.closest('.terms-check') || input?.parentElement;
  if(!holder) return;
  holder.classList.add('invalid');
  const old = holder.querySelector('.field-error');
  if(old) old.remove();
  holder.insertAdjacentHTML('beforeend', `<small class="field-error">${esc(message)}</small>`);
}
...
function applyBrand(){
  ...
  if(brand.name) document.title = `${brand.name} | شحن ألعاب وتطبيقات بسرعة وأمان`;
  const img = document.querySelector('.brand img');
  if(img) img.src = logo;
  const name = document.querySelector('.brand b');
  if(name && brand.name){
    const parts = String(brand.name).trim().split(/\s+/);
    name.innerHTML = `${esc(parts[0] || 'MOBA')} <em>${esc(parts.slice(1).join(' ') || 'SHOP')}</em>`;
  }
...
function offerHtml(o){
  ...
  return `<button class="offer-card" data-offer-game="${o.game || 'pubg'}" data-offer-section="${o.section || ''}" data-offer-products="${productIds}" type="button">
    ...
    <div class="offer-content">
      ...
      <h2>${esc(o.title || 'عروض حصرية')}</h2>
      <p>${esc(o.subtitle || 'باقات مختارة بسعر مميز')}</p>
      <span class="price">${esc(o.cta || 'اضغط لفتح العرض')}</span>
    </div>
  </button>`;
}
function gameCard(g){
  ...
  return `<button class="game-card ${soon?'soon':''}" data-game="${g.slug}" type="button">
    ...
    <div class="game-info"><h3>${esc(g.name)}</h3><p>${esc(g.subtitle || '')}</p></div>
  </button>`;
}
...
function renderGame(slug=currentGameSlug()){
  ...
  page(`
    <section class="section">
      <div class="page-title">
        <div>
          <span class="badge">${esc(game.name)}</span>
          <h1>${esc(game.name)}</h1>
    ...
    <section class="section products-layout">
      <aside class="side-list">
        ${sections.map(s=>`<button type="button" data-section="${esc(s.slug)}" class="${s.slug===active?'active':''}">${esc(s.name)}</button>`).join('')}
      </aside>
      <div>
        <div class="fields-grid" id="accountFields">
          <div class="field"><label>${esc(game.idLabel)}</label><input id="accountId" inputmode="numeric" placeholder="${esc(game.idPlaceholder)}"></div>
          <div class="field"><label>${esc(game.nameLabel)}</label><input id="accountName" placeholder="${esc(game.namePlaceholder)}"></div>
          ${(game.fields||[]).map(f=>`<div class="field"><label>${esc(f.label)}</label><input data-extra-field="${esc(f.key)}" ${f.type==='number'?'inputmode="numeric"':''} placeholder="${esc(f.placeholder||f.label)}" ${f.required?'data-required="1"':''}></div>`).join('')}
        </div>
...
function productCard(p){
  ...
  return `<article class="product-card" data-product="${p.id}">
    ${p.image ? `<img class="product-thumb" src="${esc(p.image)}" alt="" loading="lazy" decoding="async">` : ''}
    <div><h3>${esc(p.name)}</h3><span>${esc(p.unit || '')}</span>${flags.length?`<div class="product-flags">${flags.map(f=>`<b>${f}</b>`).join('')}</div>`:''}</div>
...
function showCouponMessage(text,type){ const box=$('#couponMessage'); if(box) box.innerHTML=`<div class="message ${type==='ok'?'ok':'err'}">${esc(text)}</div>`; }
...
function checkoutMessage(text,type){ const box=$('#checkoutMessage'); if(box) box.innerHTML=`<div class="message ${type==='ok'?'ok':'err'}">${esc(text)}</div>`; }
...
async function trackOrder(phone){
  ...
  }catch(e){ box.innerHTML=`<div class="message err">${esc(customerErrorMessage(e))}</div>`; }
}
...
async function renderReviews(){
  ...
  }catch(e){ $('#reviewsList').innerHTML=`<div class="message err">${esc(customerErrorMessage(e))}</div>`; }
}

function assistantReply(text, actions=[]){
  const safeText = esc(text).replace(/&lt;br\s*\/?&gt;/gi,'<br>').replace(/\n/g,'<br>');
  const box=$('#assistantMessages'); const el=document.createElement('div'); el.className='bubble'; el.innerHTML=`<div>${safeText}</div>${actions.length?`<div class="quick-actions">${actions.map(a=>`<button type="button" data-assist="${esc(a.cmd)}">${esc(a.label)}</button>`).join('')}</div>`:''}`; box.appendChild(el); box.scrollTop=box.scrollHeight;
}
...
async function render(){
  if(!catalog.games.length){ renderSkeleton(); return; }
  applyBrand();
...
function addProduct(id, card){
  ...
  if(!product) return toast('المنتج غير متاح حاليا، جرب تحديث الصفحة.', 'err');
  const game=gameBySlug(product.game);
...
async function submitFix(form){
  ...
  }catch(e){ btn.disabled=false; btn.textContent='إرسال التعديل'; form.insertAdjacentHTML('beforeend',`<div class="message err">${esc(customerErrorMessage(e))}</div>`); }
}
document.addEventListener('click',e=>{
...
async function visitorPing(){
  try{
    ...
    if(d?.blocked) document.body.innerHTML = `<main class="app-shell"><section class="section"><div class="message err">${esc(d.error || 'تم حظر الجهاز من استخدام الموقع.')}</div></section></main>`;
  }catch{}
}
visitorPing();
...
submitCheckout = async function(form){
  ...
  try{
    ...
    if(!/^01\d{9}$/.test(String(fd.get('customerPhone')||''))){
      showFieldError(form, '[name="customerPhone"]', 'اكتب رقم موبايل صحيح يبدأ بـ 01 ومكون من 11 رقم.');
      throw new Error('راجع رقم الموبايل قبل تنفيذ الطلب.');
    }
    if(fd.get('termsAccepted')!=='true'){
      showFieldError(form, '[name="termsAccepted"]', 'لازم توافق على الشروط قبل تنفيذ الطلب.');
      throw new Error('لازم توافق على شروط الشحن وسياسة الاسترجاع قبل تنفيذ الطلب.');
    }
    const warning=$('.product-warning');
    if(warning && warning.dataset.accepted!=='true') throw new Error('اقرأ تنبيه المنتج واضغط موافق ومتاكد قبل تنفيذ الطلب.');
    if(fd.get('transferMode')==='other' && !/^\d{3}$/.test(String(fd.get('transferLast3')||''))){
      showFieldError(form, '[name="transferLast3"]', 'اكتب آخر 3 أرقام فقط من رقم التحويل.');
      throw new Error('اكتب آخر 3 أرقام من رقم التحويل.');
    }
    if(!checkoutFile){
      showFieldError(form, '#screenshotInput', 'ارفع صورة إثبات الدفع JPG أو PNG أو WEBP.');
      throw new Error('ارفع سكرين التحويل الأول.');
    }
    if(!/^image\/(png|jpeg|webp)$/i.test(checkoutFile.type || '')){
      showFieldError(form, '#screenshotInput', 'الصورة غير صالحة، ارفع JPG أو PNG أو WEBP.');
      throw new Error('الصورة غير صالحة، ارفع صورة JPG أو PNG أو WEBP.');
    }
