...
function productCard(p){
  return `<article class="product-card" data-product="${p.id}">
    <div><h3>${p.name}</h3><span>${p.unit || ''}</span></div>
  ...
  page(`
    ...
    <section class="section">
      ...
      <form id="checkoutForm" class="checkout-grid" style="margin-top:14px">
        ...
        <div class="field"><label>سكرين التحويل</label><input id="screenshotInput" name="screenshot" type="file" accept="image/png,image/jpeg,image/webp" required><div id="filePreview" class="message">لم يتم اختيار صورة بعد</div></div>
...
function addProduct(id, card){
  ...
  addCartItem({ productId:product.id, product:product.name, game:game.name, price:product.price, qty, noQty:product.noQty, pubgId:accId, pubgName:accName, extraFields:extras });
...
function assistantAction(cmd){
  ...
  if(cmd.startsWith('combo:')){ navigate('/game/pubg'); assistantReply('تمام، اختار باقة PUBG من الصفحة واكتب ID واسم الحساب، وبعدها كمل الدفع.'); }
}
