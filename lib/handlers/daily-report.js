...
export default async function handler(req,res){
  ...
  try{
    ...
    const allRows=await supabaseRequest(`orders?order_date=gte.${encodeURIComponent(p.from)}&order_date=lte.${encodeURIComponent(p.to)}&select=*&order=created_at.asc&limit=1000`).catch(()=>[]);
    const rows=(allRows||[]).filter(o=>o.order_type!=='test' && o.raw_data?.testOrder!==true);
