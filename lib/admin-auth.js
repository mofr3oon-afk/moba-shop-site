...
const SESSION_TTL_SECONDS = Number(process.env.ADMIN_SESSION_TTL_SECONDS || 24 * 60 * 60);
...
export function makeSession(req, extra={}){
  ...
  const username = String(extra.username || '').slice(0,40);
  return {role, username, deviceId, ip, ua, iat:Date.now(), exp:Date.now() + SESSION_TTL_SECONDS * 1000};
}
