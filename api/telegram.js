...
async function reportText(days=1,title='تقرير'){
  ...
  const rows=(await supa(`orders?created_at=gte.${encodeURIComponent(since)}&select=*&order=created_at.desc&limit=1000`)).filter(o=>o.order_type!=='test' && o.raw_data?.testOrder!==true);
  const counts={};
  const payments={};
  const products={};
  let total=0;
...
${msgTxt || '-'}`});
  ...
  if(cmd==='/done_today'){
    const start=new Date(); start.setHours(0,0,0,0);
    const rows=(await supa(`orders?created_at=gte.${encodeURIComponent(start.toISOString())}&status=eq.delivered&select=*&order=created_at.desc&limit=30`)).filter(o=>o.order_type!=='test' && o.raw_data?.testOrder!==true);
  ...
  if(cmd==='/stats'){
    const start=new Date(); start.setHours(0,0,0,0);
    const rows=(await supa(`orders?created_at=gte.${encodeURIComponent(start.toISOString())}&select=*&order=created_at.desc&limit=1000`)).filter(o=>o.order_type!=='test' && o.raw_data?.testOrder!==true);
