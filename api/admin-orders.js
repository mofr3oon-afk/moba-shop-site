  ...
  try{
    ...
    if(req.method === 'POST'){
      ...
      if(action === 'archive_delivered'){
        requireRole(admin, ['owner','staff']);
        const rows=await supabaseRequest(`orders?status=eq.delivered&select=id,status`);
        let ok=0, fail=0;
        for(const o of rows||[]){
          try{ await supabasePatchWithSchemaFallback(`orders?id=eq.${encodeURIComponent(o.id)}`, {status:'archived',status_text:'أرشيف',admin_status_text:'أرشيف',customer_status_text:'تم حفظ الطلب في الأرشيف',last_status_by:admin?.role==='owner'?'Owner':'Staff',last_status_at:new Date().toISOString(),updated_at:new Date().toISOString(),note:'Archived delivered orders from admin panel'}); ok++; }
          catch(_){ fail++; }
        }
        await logAdminEvent('archive_delivered_orders', req, {ok,fail}).catch(()=>null);
        return json(res,200,{ok:true,count:ok,fail});
      }
      if(action === 'delete_delivered'){
        requireRole(admin, ['owner']);
        const rows=await supabaseRequest(`orders?status=eq.delivered&select=id,status`);
        const count=(rows||[]).length;
        if(count){ await supabaseRequest(`orders?status=eq.delivered`,{method:'DELETE'}); }
        await logAdminEvent('delete_delivered_orders', req, {count}).catch(()=>null);
        return json(res,200,{ok:true,count});
      }
      if(action === 'delete_test_orders'){
        requireRole(admin, ['owner']);
        if(String(body.confirmText||'') !== 'DELETE') return json(res,400,{ok:false,error:'اكتب DELETE لتأكيد حذف طلبات TEST'});
        const rows=await supabaseRequest(`orders?select=id,order_type,raw_data&limit=500`);
        const testRows=(rows||[]).filter(o=>o.order_type==='test' || o.raw_data?.testOrder===true);
        let count=0;
        for(const o of testRows){
          await supabaseRequest(`orders?id=eq.${encodeURIComponent(o.id)}`,{method:'DELETE'}).then(()=>count++).catch(()=>null);
        }
        await logAdminEvent('delete_test_orders', req, {count}).catch(()=>null);
        return json(res,200,{ok:true,count});
      }
      if(action === 'set_vip'){
        requireRole(admin, ['owner','staff']);
        ...
        const raw={...(order.raw_data||{}),vip:Boolean(body.vip)};
        await supabasePatchWithSchemaFallback(`orders?id=eq.${encodeURIComponent(id)}`, {vip:Boolean(body.vip),customer_vip:Boolean(body.vip),raw_data:raw,updated_at:new Date().toISOString()});
        await logAdminEvent('set_customer_vip', req, {orderId:id,phone:order.phone,vip:Boolean(body.vip)}).catch(()=>null);
      ...
      if(action === 'edit_order'){
        ...
        const pubgId=String(body.pubgId||'').trim();
        const pubgName=String(body.pubgName||'').trim();
        const internal_note=String(body.internal_note||'').trim();
        const items=Array.isArray(order.items)?order.items.map((it,idx)=>idx===0?{...it,pubgId:pubgId||it.pubgId,pubgName:pubgName||it.pubgName,name:pubgName||it.name}:it):order.items;
        await supabasePatchWithSchemaFallback(`orders?id=eq.${encodeURIComponent(id)}`, {
          phone,
          customer_phone:phone,
          items,
          internal_note,
          updated_at:new Date().toISOString(),
          last_status_by: admin?.username || admin?.role || 'admin',
          last_status_at:new Date().toISOString()
        });
        await logAdminEvent('edit_order_limited', req, {orderId:id,phone,pubgId,pubgName,internal_note:Boolean(internal_note)}).catch(()=>null);
        return json(res,200,{ok:true});
      }
      const id=String(body.id||'').trim();
      ...
      const adminName = admin?.username || (admin?.role === 'owner' ? 'Owner' : 'Staff');
      history.push({status,label:STATUS_LABELS[status]||status,at:new Date().toISOString(),by:adminName,role:admin?.role||'owner',username:admin?.username||''});
      const customerText=customerStatusText(status,note);
      const patch={status,status_text:STATUS_LABELS[status]||status,admin_status_text:STATUS_LABELS[status]||status,customer_status_text:customerText,last_status_by: `${adminName} (${admin?.role||'owner'})`,last_status_at:new Date().toISOString(),updated_at:new Date().toISOString(),status_history:history.slice(-50)};
      if(status === 'claimed' || status === 'processing'){
        patch.handler = adminName;
        patch.handler_id = admin?.deviceId || '';
        patch.claimed_by = admin?.deviceId || adminName;
        patch.claimed_by_name = adminName;
        if(status === 'claimed') patch.claimed_at = new Date().toISOString();
      }
      if(status === 'pending' && /إلغاء استلام|Unclaim|سحب الاستلام/i.test(note)){
        patch.handler = null;
        patch.handler_id = null;
      ...
      await logAdminEvent('order_status_update', req, {orderId:id,status,note,adminName,role:admin?.role||'owner'}).catch(()=>null);
