...
export default async function handler(req,res){
  try{ rateLimit(req,'admin-orders',60,60_000); requireAdmin(req); }
  ...
  try{
    ...
    if(req.method === 'POST'){
      ...
      history.push({status,label:STATUS_LABELS[status]||status,at:new Date().toISOString(),by:'admin_panel'});
      const patch={status,status_text:STATUS_LABELS[status]||status,admin_status_text:STATUS_LABELS[status]||status,customer_status_text:STATUS_LABELS[status]||status,last_status_by:'المسؤول',last_status_at:new Date().toISOString(),updated_at:new Date().toISOString(),status_history:history.slice(-50)};
