...
const PUBLIC_KEYS = ['store_status','store_message','store_status_message','work_status','work_status_message','work_hours','support_phone','product_overrides','dynamic_products','dynamic_sections','exclusive_offer','game_settings','policy_text','maintenance_mode','payment_settings','brand_settings'];
...
function sectionDedupeKey(raw){
  ...
  if(/شهر\s*6/.test(title) || /شهر\s*٦/.test(title) || /month\s*6/i.test(title)) title='month_6_offer';
  if(title.includes('عروض شهر') || title.includes('عرض شهر')) title='month_6_offer';
  return game+'|'+title;
}
  ...
  if(Array.isArray(input.dynamic_sections)){
    ...
    for(const raw of input.dynamic_sections){
      ...
      const sec={
        ...
        id_label: String(raw.id_label||'').trim(),
        id_placeholder: String(raw.id_placeholder||'').trim(),
        id_type: String(raw.id_type||'text').trim() || 'text',
        name_label: String(raw.name_label||'').trim(),
        name_placeholder: String(raw.name_placeholder||'').trim(),
        custom_fields: Array.isArray(raw.custom_fields) ? raw.custom_fields : (Array.isArray(raw.fields) ? raw.fields : [])
      };
      const k=game+'|'+key;
  ...
  if(Array.isArray(input.dynamic_products)){
    ...
    for(const raw of input.dynamic_products){
      ...
      const prod={
        ...
        id_placeholder: String(raw.id_placeholder||'').trim(),
        id_type: String(raw.id_type||'text').trim() || 'text',
        id_required: raw.id_required !== false,
        name_label: String(raw.name_label||'').trim(),
        name_placeholder: String(raw.name_placeholder||'').trim(),
        name_required: raw.name_required !== false,
        require_id: raw.require_id !== false,
...
export default async function handler(req,res){
  ...
  try{
    if(req.method === 'GET'){
      ...
      let settings = await readSettings(wantsAdmin);
      // v195: even on GET, hide/dedupe legacy duplicated home sections before sending to site/admin.
      settings = cleanCatalogInput(settings || {});
    ...
    if(req.method === 'POST'){
      ...
      const allowed = ['store_status','store_message','work_status','work_status_message','work_hours','support_phone','blacklist_phones','blacklist_ips','blacklist_device_ids','blacklist_pubg_ids','trusted_admin_devices','trusted_admin_ips','product_overrides','dynamic_products','dynamic_sections','exclusive_offer','game_settings','coupon_rules','maintenance_mode','payment_settings','policy_text','store_status_message','vip_phones','customer_notes','staff_settings','sla_minutes','admin_notifications','payment_settings','payment_history','trusted_admin_device_meta','blacklist_entries','client_error_logs','brand_settings'];
      ...
      if(input.payment_settings && JSON.stringify(previousSettings.payment_settings || {}) !== JSON.stringify(input.payment_settings || {})){
