...
function cleanCatalogInput(input){
  if(Array.isArray(input.dynamic_sections)){
    ...
    for(const raw of input.dynamic_sections){
      ...
      const k=game+'|'+key;
      const t=game+'|'+textKey(title);
      if(byTitle.has(t) && byTitle.get(t)!==k){
        // نفس اسم القسم لنفس اللعبة: نحدث القديم بآخر بيانات بدل ما يتكرر في الموقع
        const oldKey=byTitle.get(t);
        byKey.set(oldKey,{...byKey.get(oldKey),...sec,key:byKey.get(oldKey).key});
        continue;
      }
      byTitle.set(t,k);
