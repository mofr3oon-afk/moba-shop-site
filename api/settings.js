...
function requireAdmin(req){
  ...
  if(!expected || got !== expected){ const err=new Error('غير مصرح'); err.statusCode=401; throw err; }
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method === 'POST'){
      ...
      const allowed = ['store_status','store_message','blacklist_phones','blacklist_ips','blacklist_device_ids','blacklist_pubg_ids'];
