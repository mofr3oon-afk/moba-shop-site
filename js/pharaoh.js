...
/* moba-v205-pharaoh-pubg-assisted-order */
(function(){
  if(window.__mobaV205PharaohPubgOrder)return; window.__mobaV205PharaohPubgOrder=true;
  const PRODUCTS=[
    {name:'UC 60',price:50,uc:60,cat:'uc'},
    {name:'UC 325',price:235,uc:325,cat:'uc'},
    {name:'UC 660',price:435,uc:660,cat:'uc'},
    {name:'UC 1800',price:1120,uc:1800,cat:'uc'},
    {name:'UC 3850',price:2170,uc:3850,cat:'uc'},
    {name:'UC 8100',price:4350,uc:8100,cat:'uc'}
  ];
  const fallbackPay={
    instapay:{label:'InstaPay',user:'mofr3oon1',phone:'01061707294',name:'مؤمن'},
    wallet:{label:'Wallet',phone:'01061707294',name:'مؤمن'}
  };
  const flow={active:false,step:'',combo:null,items:[],pubgId:'',accountName:'',paymentKey:'',paymentLabel:'',screenshot:null,phone:'',busy:false};
  const qs=(s,r=document)=>r.querySelector(s);
  const qsa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  ...
  function openPanel(){try{qs('#pharaohModal')?.classList.add('show'); qs('#pharaohAssistant')?.classList.add('show')}catch(e){}}
  function add(cls,html){
    openPanel();
    const b=body(); if(!b)return;
    const m=document.createElement('div');
    m.className='pharaoh-msg '+cls+' pharaoh-v205-msg';
    m.innerHTML=html;
    const ty=qs('#pharaohTyping',b);
    if(ty&&ty.parentElement===b)b.insertBefore(m,ty); else b.appendChild(m);
    b.scrollTop=b.scrollHeight;
  ...
  function normDigits(s){return String(s||'').replace(/[٠-٩]/g,d=>'٠١٢٣٤٥٦٧٨٩'.indexOf(d)).replace(/[۰-۹]/g,d=>'۰۱۲۳۴۵۶۷۸۹'.indexOf(d))}
  ...
  function isBudgetText(text){
    const s=String(text||'');
    const n=normDigits(s);
    const a=amountFrom(n);
    if(!a||a<40||a>50000)return false;
    if(/01\d{9}/.test(n))return false;
    return /(معايا|معي|ميزاني|فلوس|رشح|ترشيح|باقة|باقه|اشحن|شحن|عايز|عاوز|جنيه|ج\b|egp|كام)/i.test(s) || /^\s*\d{2,5}\s*(ج|جنيه|egp)?\s*$/i.test(n);
  }
  function comboKey(items){
    const m=new Map();
    items.forEach(p=>m.set(p.name,(m.get(p.name)||0)+1));
    return [...m.entries()].sort().map(([n,q])=>q+'x'+n).join('+');
  }
  function comboTitle(items){
    const m=new Map();
    items.forEach(p=>m.set(p.name,(m.get(p.name)||0)+1));
    return [...m.entries()].map(([n,q])=>q>1?`${q}× ${n}`:n).join(' + ');
  }
  function buildCombos(amount){
    amount=Number(amount||0);
    const ps=PRODUCTS.filter(p=>p.price<=amount);
    ...
    function add(items){
      ...
      seen.add(key);
      out.push({items,total,left:amount-total,uc:items.reduce((s,p)=>s+p.uc,0)});
    }
    ps.forEach(p=>{
      add([p]);
      const max=Math.min(20,Math.floor(amount/p.price));
      for(let q=2;q<=max;q++)add(Array(q).fill(p));
    });
    for(let i=0;i<ps.length;i++)for(let j=i;j<ps.length;j++)for(let k=j;k<ps.length;k++){
      add([ps[i],ps[j]]);
      add([ps[i],ps[j],ps[k]]);
    }
    return out.sort((a,b)=>a.left-b.left||b.uc-a.uc||b.total-a.total).slice(0,3);
  }
  function startBudget(amount){
    const picks=buildCombos(amount);
    flow.active=false;
    if(!picks.length){
      bot(`<div class="pharaoh-pg-card"><b>ميزانية ${money(amount)}</b><small>أقل باقة PUBG عندنا 50 جنيه. زود الميزانية شوية أو اختار 60 UC.</small></div>`);
      return true;
    }
    window.__mobaV205PubgCombos=picks;
    bot(`<div class="pharaoh-pg-card"><b>أفضل 3 ترشيحات PUBG لميزانية ${money(amount)}</b><small>دوس على الترشيح اللي يعجبك، وفرعون هيكمل معاك الطلب لحد التسجيل.</small>${picks.map((c,i)=>`<button type="button" class="pharaoh-pg-pick" data-pg-combo="${i}"><b>${i+1}) ${esc(comboTitle(c.items))}</b><span>الإجمالي: ${money(c.total)}${c.left?` | المتبقي: ${money(c.left)}`:' | بدون متبقي'} | ${Number(c.uc).toLocaleString('en-US')} UC</span></button>`).join('')}<div class="pharaoh-pg-note">حاليا الترشيحات الذكية مفعلة لـ PUBG فقط عشان الطلب يتسجل صح من غير خلط ألعاب.</div></div>`);
    return true;
  }
  function aggregateItems(items){
    ...
    items.forEach(p=>{
      ...
      const row=map.get(k)||{product:p.name,type:'شحن PUBG',price:p.price,uc:p.uc,qty:0,qtyTotal:0,ucTotal:0,game:'pubg',currency:'UC',cat:'uc'};
      row.qty+=1; row.qtyTotal=row.qty; row.ucTotal=row.uc*row.qty;
      map.set(k,row);
    });
    return [...map.values()];
  }
  function selectCombo(i){
    const c=(window.__mobaV205PubgCombos||[])[Number(i)];
    if(!c)return;
    Object.assign(flow,{active:true,step:'id',combo:c,items:aggregateItems(c.items),pubgId:'',accountName:'',paymentKey:'',paymentLabel:'',screenshot:null,phone:'',busy:false});
    bot(`<div class="pharaoh-pg-card"><b>تم اختيار الترشيح</b><small>${esc(comboTitle(c.items))}<br>الإجمالي: ${money(c.total)}</small><div class="pharaoh-pg-next">اكتب PUBG ID أرقام فقط.</div></div>`);
    setPlaceholder('اكتب PUBG ID هنا...');
  }
  function setPlaceholder(t){try{const i=input(); if(i)i.placeholder=t}catch(e){}}
  function extractId(t){
    const nums=normDigits(t).match(/\d{5,15}/g)||[];
    return nums[nums.length-1]||'';
  }
  function extractPhone(t){
    const nums=normDigits(t).match(/01\d{9}/g)||[];
    return nums[nums.length-1]||'';
  }
  async function getPay(){
    try{
      if(window.__mobaV205PaySettings)return window.__mobaV205PaySettings;
      const r=await fetch('/api/settings?ts='+Date.now(),{cache:'no-store'});
      const d=await r.json();
      const p=d?.settings?.payment_settings||d?.settings?.payments||{};
      window.__mobaV205PaySettings={
        instapay:{...fallbackPay.instapay,...(p.instapay||{})},
        wallet:{...fallbackPay.wallet,...(p.wallet||{})}
      };
      return window.__mobaV205PaySettings;
    }catch(e){return fallbackPay}
  }
  async function askPayment(){
    flow.step='payment';
    const p=await getPay();
    bot(`<div class="pharaoh-pg-card"><b>اختار طريقة الدفع</b><small>بعد الاختيار هاظهرلك بيانات التحويل وبعدها ترفع السكرين.</small><div class="pharaoh-pg-actions"><button type="button" data-pg-pay="instapay">InstaPay</button><button type="button" data-pg-pay="wallet">محفظة كاش</button></div><div class="pharaoh-pg-muted">InstaPay: ${esc(p.instapay.user||'-')} | Wallet: ${esc(p.wallet.phone||'-')}</div></div>`);
  }
  async function choosePay(key){
    const p=await getPay();
    const data=p[key]||fallbackPay[key]||{};
    flow.paymentKey=key;
    flow.paymentLabel=key==='instapay'?'InstaPay':'Wallet';
    flow.step='screenshot';
    const details=key==='instapay'?`User: ${esc(data.user||'-')}<br>Phone: ${esc(data.phone||'-')}<br>Name: ${esc(data.name||'-')}`:`Phone: ${esc(data.phone||'-')}<br>Name: ${esc(data.name||'-')}`;
    bot(`<div class="pharaoh-pg-card"><b>حوّل على ${flow.paymentLabel}</b><div class="pharaoh-pg-details">${details}</div><small>بعد التحويل ارفع سكرين واضح يظهر المبلغ والوقت وبيانات التحويل.</small><label class="pharaoh-pg-upload">اختيار سكرين التحويل<input type="file" accept="image/*" data-pg-shot></label><div class="pharaoh-pg-muted">هنتقل للخطوة اللي بعدها بعد اختيار الصورة.</div></div>`);
  }
  function screenshotPicked(file){
    if(!file)return;
    flow.screenshot=file;
    flow.step='phone';
    bot(`<div class="pharaoh-pg-card"><b>وصل السكرين</b><small>${esc(file.name||'صورة التحويل')}</small><div class="pharaoh-pg-next">اكتب رقم الموبايل اللي هتتابع بيه الطلب. مثال: 01012345678</div></div>`);
    setPlaceholder('اكتب رقم الموبايل...');
  }
  function summary(){
    const total=flow.items.reduce((s,x)=>s+Number(x.price||0)*Number(x.qty||1),0);
    flow.step='confirm';
    bot(`<div class="pharaoh-pg-card"><b>راجع الطلب قبل الإرسال</b><div class="pharaoh-pg-details">اللعبة: PUBG Mobile<br>ID: ${esc(flow.pubgId)}<br>الاسم: ${esc(flow.accountName)}<br>الباقات: ${esc(flow.items.map(x=>(x.qty>1?x.qty+'× ':'')+x.product).join(' + '))}<br>الدفع: ${esc(flow.paymentLabel)}<br>رقم المتابعة: ${esc(flow.phone)}<br>الإجمالي: ${money(total)}<br>السكرين: ${esc(flow.screenshot?.name||'مرفوع')}</div><div class="pharaoh-pg-actions"><button type="button" class="gold" data-pg-confirm>تأكيد وإرسال الطلب</button><button type="button" data-pg-cancel>إلغاء</button></div></div>`);
  }
  async function submitOrder(){
    if(flow.busy)return;
    if(!flow.screenshot){bot('<div class="pharaoh-pg-card"><b>ناقص السكرين</b><small>ارفع سكرين التحويل الأول.</small></div>');return}
    flow.busy=true;
    bot('<div class="pharaoh-pg-card"><b>جاري تسجيل الطلب...</b><small>استنى ثواني، هابعت الطلب للوحة الأدمن والتليجرام.</small></div>');
    const cart=flow.items.map(x=>({...x,pubgId:flow.pubgId,pubgName:flow.accountName}));
    const fd=new FormData();
    fd.append('cart',JSON.stringify(cart));
    fd.append('customerPhone',flow.phone);
    fd.append('paymentMethod',flow.paymentLabel);
    fd.append('transferMode','same');
    fd.append('transferLast3','');
    fd.append('note','طلب مكتمل من فرعون المساعد');
    fd.append('deviceId',localStorage.getItem('moba_device_id')||localStorage.getItem('mobaDeviceId')||'pharaoh-assistant');
    fd.append('screenshot',flow.screenshot,flow.screenshot.name||'pharaoh-screenshot.jpg');
    try{
      const r=await fetch('/api/order',{method:'POST',body:fd});
      const d=await r.json().catch(()=>({ok:false,error:'رد غير مفهوم من السيرفر'}));
      if(!d.ok)throw new Error(d.error||'تعذر تسجيل الطلب');
      bot(`<div class="pharaoh-pg-card success"><b>تم تسجيل الطلب بنجاح</b><small>رقم الطلب: ${esc(d.orderCode||d.orderId||'MOBA')}<br>رقم المتابعة: ${esc(d.phone||flow.phone)}<br>الإجمالي: ${money(d.total||flow.items.reduce((s,x)=>s+x.price*x.qty,0))}</small><div class="pharaoh-pg-next">الطلب دخل المراجعة اليدوية، وبعد التأكد من الدفع والبيانات هيتم التنفيذ.</div><div class="pharaoh-pg-actions"><button type="button" data-pg-track="${esc(flow.phone)}">تابع الطلب</button><button type="button" data-pg-new>طلب جديد</button></div></div>`);
      Object.assign(flow,{active:false,step:'',busy:false});
    }catch(err){
      flow.busy=false;
      bot(`<div class="pharaoh-pg-card error"><b>الطلب متسجلش</b><small>${esc(err.message||'حصل خطأ')}</small><div class="pharaoh-pg-next">راجع البيانات أو كلم الدعم لو المشكلة مستمرة.</div></div>`);
    }
  }
  function handleFlow(text){
    if(!flow.active)return false;
    if(/^(الغاء|إلغاء|cancel)$/i.test(String(text||'').trim())){
      Object.assign(flow,{active:false,step:'',busy:false});
      bot('<div class="pharaoh-pg-card"><b>تم إلغاء الخطوات</b><small>تقدر تبدأ من جديد في أي وقت.</small></div>');
      return true;
    }
    if(flow.step==='id'){
      const id=extractId(text);
      if(!id){bot('<div class="pharaoh-pg-card error"><b>ID مش واضح</b><small>اكتب PUBG ID أرقام فقط، من 5 لـ 15 رقم.</small></div>');return true}
      flow.pubgId=id; flow.items=flow.items.map(x=>({...x,pubgId:id}));
      bot('<div class="pharaoh-pg-card"><b>تمام، هات اسم الحساب</b><small>اكتب اسم الحساب زي ما ظاهر داخل اللعبة.</small></div>');
      flow.step='name'; setPlaceholder('اكتب اسم الحساب...');
      return true;
    }
    if(flow.step==='name'){
      const name=String(text||'').trim();
      if(name.length<2){bot('<div class="pharaoh-pg-card error"><b>الاسم قصير</b><small>اكتب اسم الحساب بشكل أوضح.</small></div>');return true}
      flow.accountName=name; flow.items=flow.items.map(x=>({...x,pubgName:name}));
      askPayment();
      return true;
    }
    if(flow.step==='phone'){
      const phone=extractPhone(text);
      if(!phone){bot('<div class="pharaoh-pg-card error"><b>رقم الموبايل مش صحيح</b><small>اكتب رقم مصري 11 رقم يبدأ بـ 01.</small></div>');return true}
      flow.phone=phone;
      summary();
      return true;
    }
    bot('<div class="pharaoh-pg-card"><b>اختار من الأزرار الظاهرة قدامك</b><small>ولو عايز تلغي اكتب: الغاء</small></div>');
    return true;
  }
  function interceptText(withUser){
    const i=input(); if(!i)return false;
    const text=i.value.trim(); if(!text)return false;
    if(flow.active){
      if(withUser)user(text);
      i.value='';
      return handleFlow(text);
    }
    if(isBudgetText(text)){
      if(withUser)user(text);
      i.value='';
      return startBudget(amountFrom(text));
    }
    return false;
  }
  document.addEventListener('click',function(e){
    const send=e.target.closest&&e.target.closest('#pharaohSendBtn,#pharaohChatForm button[type="submit"],#pharaohChatForm button:not(#pharaohMicBtn)');
    if(send&&interceptText(true)){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();return false}
    const pick=e.target.closest&&e.target.closest('[data-pg-combo]');
    if(pick){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();selectCombo(pick.dataset.pgCombo);return false}
    const pay=e.target.closest&&e.target.closest('[data-pg-pay]');
    if(pay){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();choosePay(pay.dataset.pgPay);return false}
    if(e.target.closest&&e.target.closest('[data-pg-confirm]')){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();submitOrder();return false}
    if(e.target.closest&&e.target.closest('[data-pg-cancel]')){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation&&e.stopImmediatePropagation();Object.assign(flow,{active:false,step:'',busy:false});bot('<div class="pharaoh-pg-card"><b>تم إلغاء الطلب</b></div>');return false}
    const tr=e.target.closest&&e.target.closest('[data-pg-track]');
    if(tr){e.preventDefault(); const ph=tr.dataset.pgTrack||flow.phone; try{if(typeof window.mobaShowView==='function')window.mobaShowView('track'); const inp=qs('#trackPhone'); if(inp)inp.value=ph; qs('#trackOrder')?.scrollIntoView({behavior:'smooth',block:'start'});}catch(_){} return false}
    if(e.target.closest&&e.target.closest('[data-pg-new]')){e.preventDefault();Object.assign(flow,{active:false,step:'',busy:false});bot('<div class="pharaoh-pg-card"><b>اكتب ميزانيتك</b><small>مثال: معايا 500 جنيه</small></div>');return false}
  },true);
  document.addEventListener('change',function(e){
    const f=e.target&&e.target.matches&&e.target.matches('[data-pg-shot]')?e.target.files&&e.target.files[0]:null;
    if(f){screenshotPicked(f)}
  },true);
  window.addEventListener('keydown',function(e){
    if(e.target&&e.target.id==='pharaohChatInput'&&e.key==='Enter'&&interceptText(true)){
      e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation&&e.stopImmediatePropagation(); return false;
    }
  },true);
  const st=document.createElement('style');
  st.textContent=`
    .pharaoh-pg-card{border:1px solid rgba(39,216,255,.28);background:linear-gradient(135deg,rgba(8,24,42,.96),rgba(18,26,44,.94));border-radius:18px;padding:13px;display:grid;gap:9px;color:#fff;box-shadow:0 14px 30px rgba(0,0,0,.24)}
    .pharaoh-pg-card b{font-size:16px}.pharaoh-pg-card small{color:#cfe6f4;line-height:1.8}.pharaoh-pg-card.success{border-color:rgba(91,255,169,.45);background:linear-gradient(135deg,rgba(10,52,39,.96),rgba(8,24,42,.94))}
    .pharaoh-pg-card.error{border-color:rgba(255,107,134,.48);background:linear-gradient(135deg,rgba(55,12,31,.96),rgba(8,24,42,.94))}
    .pharaoh-pg-pick{width:100%;text-align:right;border:1px solid rgba(255,214,108,.32);background:rgba(255,214,108,.08);color:#fff;border-radius:15px;padding:11px;cursor:pointer;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease}
    .pharaoh-pg-pick:hover{transform:translateY(-2px);border-color:rgba(255,214,108,.72);box-shadow:0 12px 26px rgba(0,0,0,.24)}.pharaoh-pg-pick:active{transform:translateY(1px) scale(.99)}
    .pharaoh-pg-pick b,.pharaoh-pg-pick span{display:block}.pharaoh-pg-pick span{color:#ffe28a;font-size:12px;margin-top:4px;line-height:1.6}
    .pharaoh-pg-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.pharaoh-pg-actions button{border:1px solid rgba(39,216,255,.28);background:rgba(255,255,255,.07);color:#fff;border-radius:12px;padding:10px;font-weight:900;cursor:pointer}.pharaoh-pg-actions .gold,.pharaoh-pg-actions button:first-child{background:linear-gradient(135deg,#ffd95e,#35dcff);color:#06101d}
    .pharaoh-pg-note,.pharaoh-pg-next,.pharaoh-pg-muted,.pharaoh-pg-details{border:1px solid rgba(39,216,255,.20);background:rgba(0,0,0,.18);border-radius:13px;padding:10px;color:#dff7ff;line-height:1.8}
    .pharaoh-pg-muted{font-size:12px;color:#a9c4d4}.pharaoh-pg-upload{display:grid;gap:8px;border:1px dashed rgba(39,216,255,.42);border-radius:14px;padding:12px;background:rgba(8,18,34,.74);font-weight:900;cursor:pointer}.pharaoh-pg-upload input{display:block;width:100%;margin-top:4px;color:#fff}
    @media(max-width:520px){.pharaoh-pg-actions{grid-template-columns:1fr}.pharaoh-pg-card{padding:11px}.pharaoh-pg-card b{font-size:15px}}
  `;
  document.head.appendChild(st);
})();

/* moba-v203-final-clickable-pharaoh-recommendations */
(function(){
  if(window.__mobaV203ClickablePharaohRecommendations)return; window.__mobaV203ClickablePharaohRecommendations=true;
  function qs(s,r=document){return r.querySelector(s)}
  function bot(html){
    const body=qs('#pharaohChatBody'); if(!body)return;
    const m=document.createElement('div'); m.className='pharaoh-msg bot pharaoh-v203-pick-msg'; m.innerHTML=html;
    const typing=qs('#pharaohTyping'); if(typing&&typing.parentElement===body)body.insertBefore(m,typing); else body.appendChild(m);
    body.scrollTop=body.scrollHeight;
  }
  function openGame(game){
    game=String(game||window.activeGame||'pubg');
    window.activeGame=game;
    window.activeCat={pubg:'uc',freefire:'diamonds',blood_mena:'blood_diamonds',mobile_legends:'ml_diamonds',yalla_ludo:'yalla_diamonds'}[game]||window.activeCat||'uc';
    try{if(typeof window.mobaShowView==='function')window.mobaShowView('game'); else location.hash='#productsSection'}catch(_){location.hash='#productsSection'}
    setTimeout(()=>{try{window.renderProducts&&window.renderProducts();}catch(_){} qs('#productsSection')?.scrollIntoView({behavior:'smooth',block:'start'});},60);
  }
  function chooseLegacyCombo(card){
    const api=window.__pharaohV91;
    const v=card.dataset.v||card.dataset.v90Combo||card.dataset.v181PickCombo;
    if(api&&typeof api.handle==='function'&&(card.dataset.v91Act==='combo'||card.dataset.v90Combo!=null)){
      api.handle('combo',v||0);
      return;
    }
    const game=card.dataset.game||window.activeGame||'pubg';
    openGame(game);
    bot('<div class="pharaoh-v85-card"><b>تمام اخترت الترشيح ✅</b><small>فتحتلك منتجات اللعبة. اختار نفس الباقة أو المجموعة الظاهرة في الترشيح، ولو عايز أكملك خطوة بخطوة اكتب ID الحساب هنا.</small></div>');
    const input=qs('#pharaohChatInput'); if(input){input.placeholder='اكتب ID الحساب هنا...'; input.focus();}
  }
  window.addEventListener('click',function(e){
    const v186=e.target.closest&&e.target.closest('[data-v186-pick]');
    if(v186)return;
    const card=e.target.closest&&e.target.closest('.pharaoh-v90-combo,.pharaoh-v182-combo');
    if(!card)return;
    e.preventDefault(); e.stopPropagation(); e.stopImmediatePropagation&&e.stopImmediatePropagation();
    chooseLegacyCombo(card);
    return false;
  },true);
  const st=document.createElement('style');
  st.textContent='.pharaoh-v90-combo,.pharaoh-v182-combo,.pharaoh-v186-combo{cursor:pointer!important;position:relative!important;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease!important}.pharaoh-v90-combo:hover,.pharaoh-v182-combo:hover,.pharaoh-v186-combo:hover{transform:translateY(-2px)!important;border-color:rgba(255,216,107,.72)!important;box-shadow:0 12px 28px rgba(0,0,0,.22)!important}.pharaoh-v90-combo:active,.pharaoh-v182-combo:active,.pharaoh-v186-combo:active{transform:translateY(1px) scale(.99)!important}.pharaoh-v90-combo .pick-hint,.pharaoh-v182-combo .pick-hint{display:block!important;color:#ffe28a!important;margin-top:5px!important;font-weight:900!important}';
  document.head.appendChild(st);
})();
