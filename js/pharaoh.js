...
(function(){
  ...
  function askPrizeRoad(){
    bot('<div class="pharaoh-v85-card"><b>عايز تشحن طريق الجايزة؟</b><small>فرعون هيكمل معاك نفس خطوات الشحن: ID، اسم الحساب، الدفع، السكرين، وتأكيد الطلب.</small><div class="pharaoh-v85-note">لو السعر مختلف عندك من لوحة الأدمن، اختار المنتج من قسم العروض. هنا هبدأ لك طلب طريق الجايزة مباشرة.</div><div class="pharaoh-v85-actions"><button type="button" class="gold" data-v169-prize-confirm>تأكيد شحن طريق الجايزة</button><button type="button" data-v91-act="manualProducts">اختار منتج تاني</button></div></div>');
  }
  ...
  document.addEventListener('click',function(e){
    ...
    if(isPrize(text)){e.preventDefault();e.stopImmediatePropagation(); if(input)input.value=''; const b=body(); if(b){const u=document.createElement('div');u.className='pharaoh-msg user pharaoh-brain-new';u.textContent=String(text||'');b.appendChild(u)} askPrizeRoad(); return false}
  },true);
  document.addEventListener('submit',function(e){
    if(!e.target||e.target.id!=='pharaohChatForm')return;
    const input=qs('#pharaohChatInput'); const text=input&&input.value;
    if(isPrize(text)){e.preventDefault();e.stopImmediatePropagation(); if(input)input.value=''; askPrizeRoad(); return false}
  },true);
})();
