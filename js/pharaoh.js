...
/* moba-v166-pharaoh-dynamic-products */
(function(){
  if(window.__mobaV166PharaohDynamicProducts)return;
  window.__mobaV166PharaohDynamicProducts=true;
  ...
  async function load(){
    ...
    try{
      ...
      state.items=Array.isArray(list)?list.filter(p=>p&&p.active!==false).map((p,i)=>({
        i,
        name:String(p.name||p.product||p.title||'عرض').trim(),
        price:Number(p.sale_price||p.price||p.amount||0),
        section:String(p.section||p.category||p.game||'العروض').trim(),
        raw:p
      })):[];
  ...
  function addBot(html){
    ...
    msg.innerHTML=html;
    const typing=qs('#pharaohTyping',body);
  ...
  async function respond(text){
    ...
    const matches=items.filter(p=>t&&norm(p.name)&&t.includes(norm(p.name))).slice(0,4);
    const budget=amount?items.filter(p=>p.price&&p.price<=amount).sort((a,b)=>b.price-a.price).slice(0,4):[];
    const list=matches.length?matches:budget;
    if(!list.length)return;
    const key=t+'|'+list.map(x=>x.name).join(',');
    if(state.seen.has(key))return;
    state.seen.add(key);
    addBot(`<div class="pharaoh-v85-card"><b>لقيتلك عروض من لوحة الأدمن</b><small>فرعون بيقرأ المنتجات الجديدة تلقائيًا من لوحة التحكم.</small>${list.map(p=>`<div class="pharaoh-v85-note"><b>${esc(p.name)}</b> - ${Number(p.price||0).toLocaleString('en-US')} جنيه<br><small>${esc(p.section)}</small></div>`).join('')}<div class="pharaoh-v85-actions"><button type="button" class="gold" data-v166-open-products>افتح المنتجات</button></div></div>`);
  }
  document.addEventListener('click',function(e){
    const btn=e.target.closest&&e.target.closest('[data-v166-open-products]');
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    new MutationObserver(muts=>{
      muts.forEach(m=>m.addedNodes&&m.addedNodes.forEach(n=>{
        if(n.nodeType===1&&n.classList&&n.classList.contains('user'))respond(n.textContent||'');
      }));
