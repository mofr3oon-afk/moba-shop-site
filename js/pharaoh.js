...
(function(){
  ...
  function bestFor(game,amount,mode){
    const list=gameProducts(game).filter(p=>p.price<=amount);
    if(mode==='cheap')return list.sort((a,b)=>a.price-b.price).slice(0,5);
    if(mode==='value')return list.sort((a,b)=>(Number(b.uc||0)/Math.max(1,b.price))-(Number(a.uc||0)/Math.max(1,a.price)) || b.price-a.price).slice(0,5);
    return list.sort((a,b)=>b.price-a.price).slice(0,5);
  }
  function recommendGame(game,amount,mode){
    ...
    saveBudget(amount);cleanupBudgetNoise();
    const name=pretty[game]||game;
    const cur=currency[game]||'';
    const picks=bestFor(game,amount,mode);
    if(!picks.length){
    ...
    return `<div class="pharaoh-v85-card pharaoh-v183-rec"><b>ترشيحات ${esc(name)} لميزانية ${money(amount)}</b><small>دي أنسب اختيارات من منتجات ${esc(name)} فقط.</small>${picks.map((p,i)=>`<div class="pharaoh-v90-combo"><b>${i+1}) ${esc(p.name||p.title||'منتج')}</b><small>السعر: <strong>${money(p.price)}</strong>${p.uc?` | القيمة: <strong>${Number(p.uc).toLocaleString('en-US')} ${esc(cur)}</strong>`:''}</small><span class="pick-hint">اختار المنتج من صفحة اللعبة أو افتح المنتجات</span></div>`).join('')}<div class="pharaoh-v85-actions"><button type="button" class="gold" data-v183-open-game="${esc(game)}">افتح منتجات ${esc(name)}</button><button type="button" data-v183-mode="cheap" data-game="${esc(game)}">أرخص حاجة</button><button type="button" data-v183-mode="value" data-game="${esc(game)}">أفضل قيمة</button><button type="button" data-v183-change-game>اختار لعبة تانية</button></div></div>`;
  }
