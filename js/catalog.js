...
function normalizeSection(raw){ return { game:raw.game, slug:raw.slug || raw.key, name:raw.name || raw.title, order:Number(raw.order ?? raw.sort_order ?? 99) }; }
function normalizeProduct(raw){
  return { id:raw.id || `p_${Date.now()}_${Math.random().toString(36).slice(2,6)}`, game:raw.game, section:raw.section || raw.cat, name:raw.name, unit:raw.unit || raw.type || '', amount:Number(raw.amount || raw.uc || 0), price:Number(raw.price || 0), order:Number(raw.order ?? raw.sort_order ?? 99), noQty:!!raw.noQty || !!raw.no_qty, status:raw.status || 'available' };
}
...
export async function loadCatalog(){
  ...
  const sections = mergeSections(DEFAULT_CATALOG.sections, [...(remote.dynamic_sections || []).map(normalizeSection), ...(local.sections || []).map(normalizeSection)].filter(s=>s.game && s.slug))
    .filter(s=>games.some(g=>g.slug===s.game)).sort((a,b)=>a.order-b.order);
  ...
  const offers = [...DEFAULT_CATALOG.offers, ...(Array.isArray(remote.exclusive_offer?.items)?remote.exclusive_offer.items:[]), ...(local.offers || [])]
    .filter(o=>o && o.active !== false).sort((a,b)=>(a.order||99)-(b.order||99));
  const brand = {...(remote.brand_settings || {}), ...(local.brand || {})};
  return {games,sections,products,offers,brand};
}
