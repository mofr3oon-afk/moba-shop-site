...
(function(){
  ...
  async function refreshStoreStatusV41(){
    try{
      ...
      const data = await res.json();
      if(data && data.ok){ try{localStorage.setItem('moba_settings_cache_v187', JSON.stringify({settings:data.settings||{},ts:Date.now()})); localStorage.setItem('moba_settings_cache', JSON.stringify(data.settings||{}));}catch(_){} applyStoreStatus(data.settings || {}); }
    }catch(e){}
  }
...
(function(){
  ...
  function paint(card,g){
    ...
    /* v197: icon handled by cart.js final renderer */
    /* v197: cover handled by cart.js final renderer */
  }
  async function sync(){
    try{
...
/* moba-v187-fast-status-from-cache */
(function(){
  if(window.__mobaV187FastStatus)return; window.__mobaV187FastStatus=true;
  function esc(v){return String(v??'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));}
  function apply(){
    try{
      const raw=localStorage.getItem('moba_settings_cache_v187')||localStorage.getItem('moba_settings_cache'); if(!raw)return;
      const d=JSON.parse(raw), s=d.settings||d;
      ...
      const cfg={available:['متاح الآن','التنفيذ شغال حاليا بشكل طبيعي.'],busy:['متاح ولكن فيه ضغط','تقدر تعمل الأوردر عادي لكن التنفيذ ممكن يتأخر شوية.'],closed:['خارج مواعيد التنفيذ','ينفع تعمل طلبك عادي دلوقتي.'],maintenance:['صيانة مؤقتة','الطلبات الجديدة متوقفة مؤقتا.']};
      const c=cfg[mode]||cfg.available; if(msg)c[1]=msg;
      document.querySelectorAll('#topStatusBadge,#headerStatusBadge,.store-status-pill,.status-pill,.live-status').forEach(el=>{if(!el||el.closest('#workStatusMini'))return; el.className='status-pill store-status-pill '+mode; el.innerHTML='<span class="status-dot"></span><span class="status-copy"><span class="status-label">'+esc(c[0])+'</span><small class="status-desc">'+esc(c[1])+'</small></span>';});
  ...
  document.addEventListener('DOMContentLoaded',apply); apply(); setTimeout(apply,50);
})();
