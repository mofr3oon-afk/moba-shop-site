...
const TOO_MANY_LOGIN_ERROR = 'محاولات كثيرة، جرب بعد قليل';
function safeAdminErrorMessage(message){
  const msg = String(message || '');
  if(/rate|limit|429|محاولات|كثيرة/i.test(msg)) return TOO_MANY_LOGIN_ERROR;
  if(/supabase|telegram|token|secret|service_role|stack|syntax|unexpected|env|bot|session|admin_panel|password/i.test(msg)){
    return 'حصلت مشكلة مؤقتة، جرب مرة تانية أو تواصل مع الدعم.';
  }
  return msg || 'حصلت مشكلة مؤقتة، جرب مرة تانية.';
}

async function api(path, opts={}){
  const r=await fetch(path,{headers:{'Content-Type':'application/json',...(opts.headers||{})},...opts});
  const d=await r.json().catch(()=>({ok:false,error:'رد غير مفهوم'}));
  if(!d.ok){ const err = new Error(safeAdminErrorMessage(d.error||'حدث خطأ')); err.status = r.status; throw err; }
  return d;
}
...
async function login(){
  ...
  catch(e){ document.querySelector('#adminPassword').value = ''; toast(e.status===429 ? TOO_MANY_LOGIN_ERROR : GENERIC_LOGIN_ERROR,'err'); }
}
async function verifyOtp(){
  try{
    ...
    content.hidden=false; loginBox.hidden=true; loginBox.style.display='none'; await loadSettings(); render();
  }catch(e){ toast(e.status===429 ? TOO_MANY_LOGIN_ERROR : GENERIC_LOGIN_ERROR,'err'); }
}
...
async function renderOrders(){
  ...
  }catch(e){ view.innerHTML=`<div class="message err">${safeAdminErrorMessage(e.message)}</div>`; }
}
...
function addProduct(game,section){
  ...
  setTimeout(()=>{
    ...
    row?.querySelector('[data-prod-field="name"]')?.focus();
  },50);
  toast('تم إضافة منتج جديد داخل القسم');
}
...
async function loadReportsLegacy(){
  ...
  try{
    const d=await api('/api/admin-orders?limit=150'); const orders=(d.orders||[]).filter(o=>!isTestOrderV218(o)); const now=Date.now();
  ...
  }catch(e){box.innerHTML=`<div class="message err">${safeAdminErrorMessage(e.message)}</div>`;}
}
...
async function loadArchive(){
  ...
  try{ const d=await api('/api/admin-orders?status=archived&limit=100'); box.innerHTML=(d.orders||[]).map(orderHtml).join('')||'<div class="notice">لا توجد طلبات مؤرشفة.</div>'; }catch(e){box.innerHTML=`<div class="message err">${safeAdminErrorMessage(e.message)}</div>`;}
}
...
async function renderAdminReviews(){
  ...
  try{
    const d=await api('/api/public?action=reviews&admin=1');
...
async function updateReview(card, action){
  try{
    await api('/api/public?action=reviews',{method:'PATCH',body:JSON.stringify({id:card.dataset.review,action,store_reply:card.querySelector('.review-reply')?.value||''})});
...
async function loadReports(){
  const box=document.querySelector('#reportsBox'); box.innerHTML='<div class="message">جاري حساب التقرير...</div>';
  try{
    const d=await api('/api/admin-orders?limit=150'); const orders=(d.orders||[]).filter(o=>!isTestOrderV218(o));
...
const renderOrdersV217 = async function(){
  ...
  }catch(e){ view.innerHTML=`<div class="message err">${escAdmin(safeAdminErrorMessage(e.message))}</div>`; }
}
...
function exportOrdersCsv(){
  const rows=filteredAdminOrders().filter(o=>!isTestOrderV218(o));
...
renderHealth = async function(){
  ...
  try{
    ...
    view.innerHTML=`<div class="message err">${escAdmin(safeAdminErrorMessage(e.message || 'تعذر فحص صحة الموقع'))}</div>`;
  }
};
