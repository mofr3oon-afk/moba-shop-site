...
async function checkLogin(){
  // لا نفتح لوحة التحكم تلقائيا من جلسة قديمة؛ الدخول لازم يعدي على خطوة التحقق كل مرة.
  try{ await api('/api/admin-login'); }catch{}
}
...
function renderOffers(){
  ...
  <div class="admin-list">${local.offers.map((o,i)=>`<div class="admin-card"><div class="row"><div class="field"><label>اسم العرض</label><input data-offer="${i}" data-offer-field="title" value="${o.title||''}"></div><div class="field"><label>وصف قصير</label><input data-offer="${i}" data-offer-field="subtitle" value="${o.subtitle||''}"></div><div class="field"><label>اللعبة التي يفتحها</label><input data-offer="${i}" data-offer-field="game" value="${o.game||'pubg'}"></div><div class="field"><label>القسم داخل اللعبة</label><input data-offer="${i}" data-offer-field="section" value="${o.section||''}"></div></div><div class="row"><div class="field"><label>صورة/بانر العرض</label><input data-offer="${i}" data-offer-field="image" value="${o.image||''}"></div><div class="field"><label>ترتيب الظهور</label><input data-offer="${i}" data-offer-field="order" value="${o.order||99}" inputmode="numeric"></div></div><div class="admin-actions"><label><input type="checkbox" data-offer="${i}" data-offer-field="active" ${o.active!==false?'checked':''}> ظاهر في الرئيسية</label><button class="mini danger" data-delete-offer="${i}">حذف العرض</button></div></div>`).join('') || '<div class="notice">لا توجد عروض مخصصة.</div>'}</div>`;
...
function renderSecurity(){
  ...
  document.querySelector('#storeStatus').value=s.store_status||'available';
  ['storeStatus','storeMessage','workHours','blacklistIps','blacklistDevices','blacklistPhones'].forEach(id=>document.querySelector('#'+id).addEventListener('change',()=>{
...
function renderReports(){
  ...
  <div class="row"><div class="field"><label>نوع التقرير</label><select id="reportRange"><option value="1">اليوم</option><option value="7">آخر أسبوع</option><option value="30">آخر شهر</option><option value="custom">يوم محدد</option></select></div><div class="field"><label>اختار تاريخ</label><input id="reportDate" type="date"></div></div>
...
async function loadReports(){
  ...
  try{
    ...
    let list=orders;
    if(range==='custom'){
      const day=document.querySelector('#reportDate')?.value;
      list=orders.filter(o=>String(o.created_at||'').slice(0,10)===day);
    }else{
