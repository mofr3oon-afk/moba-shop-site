...
async function login(){
  try{ await api('/api/admin-login',{method:'POST',body:JSON.stringify({secret:document.querySelector('#adminSecret').value,deviceId:localStorage.getItem('moba_device_id')||'admin'})}); content.hidden=false; loginBox.hidden=true; await loadSettings(); render(); }
...
async function saveSettings(){
  try{
    ...
    const payload={settings:{game_settings,dynamic_sections:local.sections||[],dynamic_products:local.products||[],exclusive_offer:{items:local.offers||[]}}};
...
function editGame(slug){
  ...
  document.querySelector('#gameEditor').innerHTML=`<h2>${game.name}</h2>
    ...
    <div class="row">
      ...
      <div class="field"><label>صورة اللعبة</label><input data-game-field="cover" value="${game.cover||''}"></div>
    </div>
