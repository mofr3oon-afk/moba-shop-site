...
function renderOffers(){
  ...
  document.querySelectorAll('[data-offer-field]').forEach(i=>i.addEventListener('change',()=>{const o=local.offers[Number(i.dataset.offer)]; const f=i.dataset.offerField; if(i.type==='checkbox') o[f]=i.checked; else if(f==='order') o[f]=Number(i.value||99); else if(f==='productIdsText') o.productIds=String(i.value||'').split(',').map(x=>x.trim()).filter(Boolean); else o[f]=i.value.trim(); saveLocalCatalog(local);}));
}
