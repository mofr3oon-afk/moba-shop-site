...
function trackCard(o,phone){
  ...
  return `<details class="track-card" data-track-status="${o.status}">
    ...
    <div class="track-detail-grid">
      <div><b>وقت الطلب</b><span>${o.created_at||'-'}</span></div>
      <div><b>طريقة الدفع</b><span>${o.payment_method||'-'}</span></div>
      <div><b>آخر تحديث</b><span>${o.updated_at||'-'}</span></div>
      <div><b>الإجمالي</b><span>${money(o.total)}</span></div>
    </div>
    <div class="track-items">${(o.items||[]).map(i=>`<article><b>${i.product}</b><span>ID: ${i.pubgId||'-'}</span><span>Name: ${i.pubgName||'-'}</span><span>Qty: ${i.qty||1}</span></article>`).join('')}</div>
