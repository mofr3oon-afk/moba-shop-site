    ...
    <section class="section checkout-section">
      ...
      <form id="checkoutForm" class="checkout-grid" style="margin-top:14px">
        ...
        <div class="pay-box payment-details">
          <b>بيانات الدفع</b>
          <p>InstaPay: ${instapay.user||'-'}</p><p>Phone: ${instapay.phone||wallet.phone||'-'}</p><p>Name: ${instapay.name||wallet.name||'-'}</p>
          <p>${instapay.message||wallet.message||''}</p>
          <a class="primary" style="display:block;text-align:center" href="${instapay.link||'#'}" target="_blank">فتح InstaPay</a>
        </div>
...
async function trackOrder(phone){
  ...
  try{ const r=await fetch('/api/status?history=1&phone='+encodeURIComponent(phone)); const d=await r.json(); if(!d.ok) throw new Error(d.error||'لا يوجد طلب');
    ...
    box.innerHTML = `<div class="track-tabs"><button data-track-filter="all">الكل <small>${groups.all.length}</small></button><button data-track-filter="fix">محتاج تعديل/معلق <small>${groups.fix.length}</small></button><button data-track-filter="done">مكتمل <small>${groups.done.length}</small></button><button data-track-filter="cancel">ملغي <small>${groups.cancel.length}</small></button></div><div id="trackOrders">${orders.map(o=>trackCard(o,phone)).join('')}</div>`;
...
function trackCard(o,phone){
  ...
  return `<details class="track-card" data-track-status="${o.status}">
    <summary><b>${o.order_code || o.id}</b><span>${o.status_label || o.status_text || o.status}</span><span>${money(o.total)}</span></summary>
    <div class="timeline"><div>وقت الطلب: ${o.created_at||'-'}</div><div>الدفع: ${o.payment_method||'-'}</div><div>آخر تحديث: ${o.updated_at||'-'}</div></div>
    <div class="admin-card">${(o.items||[]).map(i=>`${i.product} - ID: ${i.pubgId||'-'} - Name: ${i.pubgName||'-'}`).join('<br>')}</div>
...
async function renderReviews(){
  page(`<section class="section"><div class="page-title"><div><h1>آراء العملاء</h1><p>اكتب رأيك، وبعد المراجعة يظهر في الموقع.</p></div></div>
  <form id="reviewForm" class="checkout-grid">
    ...
    <button class="primary" type="submit">إرسال الرأي للمراجعة</button>
    <div id="reviewMessage"></div>
  </form></section>
  <section class="section"><h2>تقييمات ظهرت بعد المراجعة</h2><div id="reviewsList" class="reviews-grid"><div class="message">جاري تحميل الآراء...</div></div></section>`);
  try{
    const r=await fetch('/api/reviews'); const d=await r.json();
    const list=$('#reviewsList');
    list.innerHTML=(d.reviews||[]).map(rv=>`<article class="review-card"><b>${rv.customer_name}</b><span>${'★'.repeat(Number(rv.rating||5))}</span><p>${rv.review_text}</p>${rv.store_reply?`<div class="store-reply"><b>MOBA SHOP</b><p>${rv.store_reply}</p></div>`:''}</article>`).join('') || '<div class="notice">لسه مفيش آراء ظاهرة.</div>';
...
async function submitReview(form){
  ...
  try{
    ...
    const r=await fetch('/api/reviews',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
...
async function submitFix(form){
  ...
  try{
    const r=await fetch('/api/fix-order',{method:'POST',body:new FormData(form)});
...
document.addEventListener('click',e=>{
  ...
  if(tf){ const f=tf.dataset.trackFilter; document.querySelectorAll('.track-card').forEach(c=>{const s=c.dataset.trackStatus; c.style.display=(f==='all'||(f==='fix'&&['needs_fix','on_hold','pending'].includes(s))||(f==='done'&&s==='delivered')||(f==='cancel'&&['cancelled','rejected'].includes(s)))?'block':'none';}); }
});
...
function initDraggablePharaoh(){
  ...
  btn.addEventListener('pointerdown',e=>{dragging=true;moved=false;btn.setPointerCapture?.(e.pointerId);const r=btn.getBoundingClientRect();dx=e.clientX-r.left;dy=e.clientY-r.top;btn.classList.add('dragging');});
  btn.addEventListener('pointermove',e=>{if(!dragging)return;moved=true;const x=Math.max(6,Math.min(innerWidth-btn.offsetWidth-6,e.clientX-dx));const y=Math.max(6,Math.min(innerHeight-btn.offsetHeight-6,e.clientY-dy));Object.assign(btn.style,{left:x+'px',top:y+'px',right:'auto',bottom:'auto'});});
  ...
  btn.addEventListener('click',e=>{if(moved){e.preventDefault();return;} $('#assistantPanel').classList.add('open'); assistantReply('أنا معاك يا بطل. محتاج تشحن، تتابع طلب، ولا تعرف طرق الدفع؟',[{label:'ابدأ الشحن',cmd:'charge'},{label:'تابع طلبي',cmd:'track'},{label:'طرق الدفع',cmd:'checkout'},{label:'مشكلة في الطلب',cmd:'track'}]);});
}
...
async function visitorPing(){
  try{
    const r=await fetch('/api/client-log',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'visitor_ping',deviceId:deviceId(),page:location.pathname})});
