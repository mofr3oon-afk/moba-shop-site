...
(function(){
  ...
  function productsMap(){const map={}; const add=p=>{if(!p||!p.name||p.hidden)return; const cat=String(p.cat||'uc'); (map[cat]=map[cat]||[]).push({...p,game:String(p.game||'pubg'),cat,status:stNorm(p.status||'available')});}; if(window.mobaProducts&&typeof window.mobaProducts==='object')Object.values(window.mobaProducts).flat().forEach(add); (Array.isArray(s().dynamic_products)?s().dynamic_products:[]).forEach(add); Object.keys(map).forEach(cat=>{const m=new Map(); map[cat].forEach(p=>m.set((p.game||'pubg')+'|'+cat+'|'+norm(p.name),{...(m.get((p.game||'pubg')+'|'+cat+'|'+norm(p.name))||{}),...p})); map[cat]=[...m.values()].sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0)||String(a.name).localeCompare(String(b.name),'ar'));}); return map;}
  ...
  function fieldDefaults(p,game){const g=gameInfo(game); const sec=secFor(p.cat,game); const idLabel=p.id_label||sec.id_label||g.id_label||'Player ID'; const idPh=p.id_placeholder||sec.id_placeholder||g.id_placeholder||('اكتب '+idLabel); const nameLabel=p.name_label||sec.name_label||g.name_label||'اسم الحساب'; const fields=[]; if(p.require_id!==false)fields.push({key:'player_id',label:idLabel,placeholder:idPh,type:p.id_type||sec.id_type||'text',required:p.id_required!==false}); if(p.require_name!==false)fields.push({key:'account_name',label:nameLabel,placeholder:p.name_placeholder||sec.name_placeholder||'اكتب اسم الحساب داخل اللعبة',type:'text',required:p.name_required!==false});
  ...
  document.addEventListener('DOMContentLoaded',()=>{refresh(); setTimeout(refresh,500); setTimeout(refresh,1500);}); window.addEventListener('load',()=>setTimeout(refresh,80)); setInterval(()=>{renderGames();renderHomeSections();},1500);
  ...
  const css=document.createElement('style'); css.textContent=`#gamesHome .game-mini-icon-v82,#gamesHome .game-icon-mini,#gamesHome .game-square-icon-v191,#gamesHome .game-square-icon-v194,#gamesHome .game-square-icon-v196,#gamesHome .game-master-icon-v197{display:none!important}#gamesHome .game-master-icon-v198{grid-area:icon!important;width:54px!important;height:54px!important;min-width:54px!important;border-radius:15px!important;overflow:hidden!important;display:grid!important;place-items:center!important;border:1px solid rgba(76,225,255,.30)!important;background:rgba(255,255,255,.08)!important}#gamesHome .game-master-icon-v198 img{width:100%!important;height:100%!important;object-fit:cover!important}#gamesHome .game-card-content{display:grid!important;grid-template-columns:54px 1fr!important;grid-template-areas:'icon badge' 'icon title' 'icon sub'!important;gap:4px 12px!important;align-items:center!important}.moba-field-v198{display:grid!important;gap:6px;color:#dff8ff;font-weight:900}.moba-field-v198 span{font-size:12px;color:#9edfff}.product-fields.compact-fields{grid-template-columns:repeat(2,minmax(0,1fr))!important}@media(max-width:640px){.product-fields.compact-fields{grid-template-columns:1fr!important}#gamesHome .game-card-content{grid-template-columns:46px 1fr!important}#gamesHome .game-master-icon-v198{width:46px!important;height:46px!important;min-width:46px!important}}`; document.head.appendChild(css);
})();
