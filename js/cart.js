...
/* moba-v166-order-modal-scroll-fix */
(function(){
  if(window.__mobaV166OrderModalScrollFix)return;
  window.__mobaV166OrderModalScrollFix=true;
  function sync(){
    const modal=document.getElementById('orderDetailsModal');
    const open=!!(modal&&modal.classList.contains('show'));
    document.body.classList.toggle('order-modal-open-v166',open);
    document.documentElement.classList.toggle('order-modal-open-v166',open);
  }
  function close(){
    const modal=document.getElementById('orderDetailsModal');
  ...
  const oldClose=window.closeOrderDetails;
  window.closeOrderDetails=function(){
    if(typeof oldClose==='function')oldClose();
    close();
  };
  document.addEventListener('click',function(e){
    if(e.target&&e.target.closest&&e.target.closest('[data-order-modal-close], [data-order-modal-close-btn], .order-details-close')){
      setTimeout(sync,20);
    }
  },true);
  ...
  document.addEventListener('DOMContentLoaded',function(){
    const modal=document.getElementById('orderDetailsModal');
    if(modal&&'MutationObserver' in window)new MutationObserver(sync).observe(modal,{attributes:true,attributeFilter:['class']});
    sync();
  });
})();
...
(function(){
  ...
  function bind(){
    ...
    if(back)back.onclick=function(){
      const homeLink=document.querySelector('[data-view="home"], a[href="#home"]');
  ...
  async function applyCouponNow(){
    ...
    if(!code){
      if(status){status.className='coupon-status show err';status.textContent='اكتب كود الخصم الأول.'}
      return;
    }
    if(!c.length||subtotal<=0){
      if(status){status.className='coupon-status show err';status.textContent='السلة فاضية. اختار منتج الأول.'}
      return;
    }
    if(btn){btn.disabled=true;btn.textContent='جاري...'}
    try{
      const url='/api/coupon?code='+encodeURIComponent(code)+'&total='+encodeURIComponent(subtotal)+'&cart='+encodeURIComponent(JSON.stringify(c));
      ...
      const saved={code:cp.code||code,discount_amount:Number(cp.discount_amount||cp.discount||0),type:cp.discount_type||'fixed'};
      try{localStorage.setItem('moba_coupon',JSON.stringify(saved));window.appliedCoupon=saved}catch(e){}
