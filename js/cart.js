...
/* moba-v166 dynamic categories + exclusive offer frontend */
(function(){
  if(window.__mobaV166Front)return; window.__mobaV166Front=true;
  const BASE=[{id:'uc',label:'شدات UC',subtitle:'باقات الشحن'},{id:'growth',label:'ازدهار',subtitle:'عروض خاصة'},{id:'prime',label:'برايم',subtitle:'اشتراكات'}];
  let settings=null;
  function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
  function money(n){return Number(n||0).toLocaleString('en-US')+' جنيه'}
  async function fetchSettings(){
    try{const r=await fetch('/api/settings?v166=' + Date.now(),{cache:'no-store'});const d=await r.json();if(d&&d.ok){settings=d.settings||{};window.mobaDynamicCategories=Array.isArray(settings.dynamic_categories)?settings.dynamic_categories:[];window.mobaFeaturedOffer=settings.featured_offer||null;}}
    catch(e){}
  }
  function allCats(){
    const map=new Map();
    BASE.forEach((c,i)=>map.set(c.id,{...c,active:true,sort_order:i,base:true}));
    (window.mobaDynamicCategories||[]).forEach((c,i)=>{if(c&&c.id&&c.active!==false)map.set(String(c.id),{...c,sort_order:Number(c.sort_order??(i+10))})});
    const prods=window.mobaProducts||{};
    Object.keys(prods).forEach((id,i)=>{if(!map.has(id))map.set(id,{id,label:id,subtitle:'منتجات',active:true,sort_order:50+i})});
    return [...map.values()].filter(c=>c.active!==false).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
  }
  function renderTabs(){
    const tabs=document.querySelector('.product-tabs-v119');
    if(!tabs) return;
    const cats=allCats().filter(c=>(window.mobaProducts?.[c.id]||[]).length || c.base || !window.mobaProducts);
    tabs.classList.add('dynamic-v166');
    tabs.innerHTML=cats.map(c=>`<button class="tab ${String(window.activeCat||'uc')===String(c.id)?'active':''}" data-cat="${esc(c.id)}">${c.image?`<img src="${esc(c.image)}" alt="">`:''}<span>${esc(c.label||c.id)}</span><small>${esc(c.subtitle||'')}</small></button>`).join('');
    if(!cats.some(c=>String(c.id)===String(window.activeCat||'uc')) && cats[0]) window.activeCat=cats[0].id;
  }
  function ensureOffer(){
    if(document.getElementById('exclusiveOfferV166')) return document.getElementById('exclusiveOfferV166');
    const el=document.createElement('section');
    el.id='exclusiveOfferV166';
    el.className='exclusive-offer-v166 moba-view';
    el.setAttribute('data-page','home');
    const anchor=document.getElementById('workStatusMini')||document.getElementById('gamesHome');
    if(anchor && anchor.parentNode) anchor.parentNode.insertBefore(el, anchor.nextSibling);
    return el;
  }
  function flatProducts(){return Object.values(window.mobaProducts||{}).flat().filter(Boolean)}
  function renderOffer(){
    const offer=window.mobaFeaturedOffer||{};
    const el=ensureOffer(); if(!el) return;
    if(offer.enabled===false){el.classList.remove('show');return;}
    const all=flatProducts();
    const names=Array.isArray(offer.products)?offer.products.map(String):[];
    let list=names.length?all.filter(p=>names.includes(String(p.name))):all.filter(p=>p.hot||p.featured).slice(0,4);
    if(!list.length && offer.cat) list=(window.mobaProducts?.[offer.cat]||[]).slice(0,4);
    if(!list.length){el.classList.remove('show');return;}
    el.classList.add('show');
    const cat=offer.cat || list[0]?.cat || window.activeCat || 'uc';
    el.innerHTML=`<div class="exclusive-inner-v166">
      <img class="exclusive-img-v166" src="${esc(offer.image||'assets/moba-shop-logo-512.webp')}" alt="${esc(offer.title||'عروض حصرية')}">
      <div class="exclusive-copy-v166"><b>${esc(offer.title||'عروض حصرية')}</b><p>${esc(offer.subtitle||'اختار العرض المناسب لفترة محدودة')}</p><div class="exclusive-products-v166">${list.slice(0,5).map(p=>`<span class="exclusive-chip-v166">${esc(p.name)} - ${money(p.price)}</span>`).join('')}</div></div>
      <button class="exclusive-btn-v166" type="button" data-v166-offer-cat="${esc(cat)}">شوف العروض</button>
    </div>`;
  }
  document.addEventListener('click',function(e){
    const b=e.target.closest('[data-v166-offer-cat]'); if(!b)return;
    e.preventDefault();
    window.activeCat=b.dataset.v166OfferCat||'uc';
    if(typeof window.mobaShowView==='function') window.mobaShowView('game'); else location.hash='#game';
    if(typeof window.renderProducts==='function') window.renderProducts();
    setTimeout(()=>document.getElementById('productsSection')?.scrollIntoView({behavior:'smooth',block:'start'}),80);
  });
  const oldRender=window.renderProducts;
  if(typeof oldRender==='function'){
    window.renderProducts=function(){renderTabs(); const out=oldRender.apply(this,arguments); setTimeout(()=>{renderTabs();renderOffer();},30); return out;}
  }
  function boot(){fetchSettings().then(()=>{renderTabs(); if(typeof window.renderProducts==='function') window.renderProducts(); setTimeout(renderOffer,350);});}
